...
app.post('/api/categories', protect, async (req, res, next) => {
  try {
...
// --- TRANSACTION ROUTES ---
app.get('/api/transactions/export/csv', protect, async (req, res, next) => {
  try {
...
app.post('/api/transactions', protect, async (req, res, next) => {
  try {
...
app.put('/api/transactions/:id', protect, async (req, res, next) => {
  try {
...
app.delete('/api/transactions/:id', protect, async (req, res, next) => {
  try {
    const transaction = await Transaction.findById(req.params.id);
    if (!transaction) return res.status(404).json({ message: 'Transaction not found' });
    if (transaction.userId.toString() !== req.user._id.toString()) return res.status(403).json({ message: 'Not authorized' });
...
// --- BUDGET ROUTES ---
app.get('/api/budgets', protect, async (req, res, next) => {
  try {
    const now = new Date();
...
app.post('/api/budgets', protect, async (req, res, next) => {
  try {
...
app.put('/api/budgets/:id', protect, async (req, res, next) => {
  try {
